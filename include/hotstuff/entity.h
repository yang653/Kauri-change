 ...
 * you may not use this file except in compliance with the License.检测到 #include 错误。请更新 includePath。已为此翻译单元(/home/yang/VScode_C++/C project/Kauri-Public-latest/src/hotstuff.cpp)禁用波形曲线。C/C++(1696)
...
class Block {
    ...
    quorum_cert_bt qc;/*目前区块高度最高的块的qc,即QCanc*/
    // quorum_cert_bt QCanc;/*区块祖先的qc*/
    bytearray_t extra;
    //std::vector<NewView> NVset; 
    //std::vector<salticidae::BoxObj<salticidae::ThreadCall>> NVset;
    std::unordered_map<ReplicaID,NewView> NVset;
    /* the following fields can be derived from above */
    uint256_t hash;
    std::vector<block_t> parents;
    block_t qc_ref;/*对法定证书的引用,即区块高度最高的块*/
