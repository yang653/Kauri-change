...
HotStuffCore::HotStuffCore(ReplicaID id,
        ...
        into_newview(false),finish_material(false),
        tails{b0},
        vote_disabled(false),
        id(id),
        NVset_t(b0->NVset),
...
std::vector<block_t> HotStuffCore::FindEquivProof(const block_t &Bparents, const block_t &Banc, std::unordered_map<ReplicaID,NewView> &Nvset)
{
    std::vector<block_t> c;
